...
Optional reasoning can use a model you run on loopback or a separately signed-in CLI such as OpenCode. CLI models can send your typed intent and bounded catalog candidates to their cloud provider; OpenCode's economical `openai/gpt-6-luna` option uses OpenAI. The app does not ship a model, endpoint, or credential. See [REASONING.md](REASONING.md) for setup, costs, data boundaries, and disabling/resetting. Never paste a secret into a search or configuration field. Check the separate provider's privacy policy as well.

When reasoning is off in the native app, choose **Configure reasoning** in Find to open and focus **Settings → Optional reasoning**. Search continues to work without a model or account. Settings also reports desktop readiness from the loaded runtime capabilities: native copy requires Wayland and `wl-copy`, while insertion requires Hyprland and `wtype`. “Ready” means the required capability was reported, not that a clipboard or insertion operation has already succeeded. Browser copy remains permission-dependent and browser insertion is unsupported.
